...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Get today's WFH records — WFH people should not be marked absent
    const { data: wfhToday } = await db.from('wfh').select('user_id').eq('date', todayStr);
    const wfhIds = new Set((wfhToday || []).map(w => w.user_id));

    // Find employees who haven't checked in and aren't on leave or WFH
    const absentees = nonExempt.filter(p => {
      ...
      if (wfhIds.has(p.id)) return false;
      return true;
    });
